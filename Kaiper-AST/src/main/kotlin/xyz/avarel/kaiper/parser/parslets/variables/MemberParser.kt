...
class MemberParser : BinaryParser(Precedence.DOT) {
    ...
    override fun parse(parser: ExprParser, left: Expr, token: Token): Expr {
        ...
        return if (parser.match(TokenType.ASSIGN)) {
            AssignmentExpr(token.position, left, name.string, parser.parseExpr())
        } else Identifier(token.position, left, name.string)
